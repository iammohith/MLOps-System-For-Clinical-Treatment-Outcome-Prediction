...
## 5. API Design
*(Not applicable for the Data module)*

---

## 6. Execution Flow
*(Detailed in `pipelines/README.md`)*

---

## 7. Infrastructure & Deployment

### S3 Backend Storage (Remote)
DVC can be configured to use S3, GCS, or Azure Blob Storage as the remote origin, decoupling massive data files from Git constraints.

```bash
# Example remote setup (Not active by default)
dvc remote add -d s3remote s3://mlops-clinical-bucket
```
...
| :--- | :--- | :--- |
| `AWS_ACCESS_KEY_ID` (Optional) | Security | Cloud storage read/write access. |
| `AWS_SECRET_ACCESS_KEY` (Optional) | Security | Cloud storage read/write access. |
